                    ...
                    <form method="POST" action="{{ route('notes.update', $note) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="content" name="content" rows="10" required
                                      class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('content', $note->content) }}</textarea>
                        ...
                        <div>
                            ...
                            <select id="folder_id" name="folder_id"
                                ...
                                @foreach ($folders as $folder)
                                    <option value="{{ $folder->id }}" @selected(old('folder_id', $note->folder_id) == $folder->id)>
                                        {{ $folder->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="tags" class="mt-1 block w-full" type="text" name="tags"
                                          :value="old('tags', $note->tags->pluck('name')->join(', '))"
                                          placeholder="{{ __('Comma separated, e.g. todo, idea') }}" />
                        ...
                        <div class="flex items-center gap-3">
                            <input id="is_public" type="checkbox" name="is_public" value="1"
                                   @checked(old('is_public', $note->is_public))
